...
- 编辑器左右分栏同步滚动（issue #30）：工作日报查看页面中，左侧等宽源码
  ...
  （不可滚动）时另一侧保持不动；同步跳转带防回环保护，不会触发反向再同步
  造成死循环。新增 widget 测试 `editor_page_sync_scroll_test.dart` 5 个
  （左→右比例同步、右→左反向同步、防回环、短内容、空内容）。

- 自动更新下载完成后自动弹出重启对话框（issue #29）：点击「检查更新」或
  启动时自动检查发现新版后，最新版后台下载完成（sha256 校验、manifest 落盘）
  即自动弹出对话框提示用户重启，用户可选择「立即重启」或「稍后重启」——
  「立即重启」走原有平台化安装流程（Linux AppImage 原子替换 / macOS dmg
  覆盖 / Windows NSIS 静默安装），环境不支持或更新包缺失时对话框内给出
  明确错误原因；「稍后重启」关闭对话框，设置页「重启并更新」按钮仍可手动
  触发。弹窗以更新阶段转为 ready 为触发条件：同一次下载完成不重复弹窗，
  稍后再次检查并下载完成会重新提示；下载完成时自动切回主窗口（随手记录
  弹窗/隐藏到托盘场景下对话框可见）。新增测试 10 个：
  `update_restart_dialog_test.dart` 10 个（下载完成自动弹窗、稍后重启不
  重复弹、立即重启调用 restartToUpdate、立即重启失败错误提示、下载失败不
  弹窗、同版本再次下载完成重新弹窗、本地开发构建不弹窗、重复触发防重入、
  下载完成恢复主窗口、无 manifest 时 restartToUpdate 返回 false）
...
- 修复左右分栏同步滚动的右侧闪烁与底部无法上滚（issue #30 人工反馈）：原
  ...
  目标侧正在进行的拖动/惯性滚动，滚到底部后难以向上滚动。修复：同步比例
  钳制到 [0, 1]（源侧越界回弹时目标停在边界，不再越界）；目标侧正处于用户
  拖动/惯性滚动（isScrollingNotifier 为 true）时跳过同步，不与用户手势互相
  打架。新增回归测试 3 个（`editor_page_sync_scroll_test.dart`：越界回弹
  目标不越界、底部可向上滚动、惯性滚动不被同步强行打断）

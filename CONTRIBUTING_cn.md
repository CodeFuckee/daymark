...
依赖与常用命令详见 [README_cn.md](README_cn.md#开发)。快速检查清单：
...
5. 所有改动同步更新 [CHANGELOG.md](CHANGELOG.md) 与 [CHANGELOG_cn.md](CHANGELOG_cn.md)（中英两份同步维护）。
...
所有文档文件均维护两份，一份英文、一份中文，中文版文件名加 `cn` 后缀
（如 `README.md` / `README_cn.md`）。改动其一需同步更新另一份。GitHub 的
Issue/PR 模板同样遵循该规则（中文模板带 `cn` 后缀）。`LICENSE` 保持单份
（MIT 标准法律文本）。

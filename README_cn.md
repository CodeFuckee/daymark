...
基于 [DESIGN_cn.md](DESIGN_cn.md) 设计方案实现。

## 功能

- **随手记录**：全局热键（默认 `Ctrl/Cmd+Shift+L`）随时弹窗记一条，回车即存
- **自动收集**：GitLab/GitHub 提交、目录文件变更、会议音频转录自动采集；每个代码实例可选择将哪些仓库的提交并入日报（issue #31）
- **AI 汇总**：供应商动态列表（Claude / DeepSeek / Ollama / OpenAI 兼容，参考 cc-switch 设计可添加任意供应商），配置时可一键通过官方 API 拉取模型下拉选择（issue #27），失败自动降级（issue #25）

...
- 历史日期补扫（issue #19）额外做 git 历史恢复（issue #32）：文件在目标日修改、
  之后又被更晚修改覆盖 mtime 时，mtime 扫描恢复不了（11 号改、14 号又改，16 号
  生成 11 号日报看不到该文件）；补扫时发现监控目录内的 git 仓库，按 author date
  查询目标日提交触及的文件补为变更记录——git 历史不随磁盘 mtime 覆盖而丢失；
  mtime 记录优先、git 记录只补缺失路径，删除提交补 kind='remove' 记录，
  git 不可用/非仓库/超时优雅跳过
